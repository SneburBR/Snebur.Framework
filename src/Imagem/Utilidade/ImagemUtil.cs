...
public partial class ImagemUtil
{
    ...
    public static BitmapSource? RetornarMiniatura(
        string caminhoArquivo,
        double alturaMaxima = ConstantesImagemApresentacao.ALTURA_IMAGEM_MINIATURA,
        bool erroRetornarNull = false)
    {
    ...
    {
        var streamRecurso = typeof(ImagemUtil).Assembly.GetManifestResourceStream("Snebur.Imagem.Recursos.Imagens.sem_imagem.jpg");
        if (streamRecurso is null)
        {
            throw new Exception($" Recurso de imagem 'sem_imagem.jpg' não encontrado na assembly {typeof(ImagemUtil).Assembly.FullName}");
        }
        return StreamUtil.RetornarMemoryStream(streamRecurso);
    }
    ...
    {
        var streamRecurso = typeof(ImagemUtil).Assembly.GetManifestResourceStream("Snebur.Imagem.Recursos.Imagens.sem_imagem.png");
        if (streamRecurso is null)
        {
            throw new Exception($" Recurso de imagem 'sem_imagem.png' não encontrado na assembly {typeof(ImagemUtil).Assembly.FullName}");
        }
        return StreamUtil.RetornarMemoryStream(streamRecurso);
    }
