...
namespace Snebur.Computador
{
    ...
    public class AcessoCompartilhamentoRede : IDisposable
    {
        ...
        private void ConectarCompartilhamento(string nomeComputadorUnc, string usuario, string senha, bool usuarioPrompt)
        {
            ...
            if (resultado != NO_ERROR && resultado != USUARIO_JA_CONECTADO)
            {
                ...
                {
                    if (!usuario.Contains(@"\") && !IpUtil.IsIP(this.NomeComputador))
                    {
