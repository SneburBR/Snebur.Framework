...
namespace Snebur.Comunicacao.WebSocket.Experimental.Handlers.WebSocket.rfc6455
{
    ...
    public class DataFrame : WebSocket.DataFrame
    {
        ...
        {
            if (this.Format == DataFormat.Raw)
            {
                this._header.PayloadSize = Length;
                switch (State)
                {
                    case DataState.Pong:
                        this._header.OpCode = OpCode.Pong;
                        //Setup Opcode for Pong frame if application has specified that we're sending a pong.
                        break;
                }
                byte[] headerBytes = this._header.ToBytes(IsByte);
        ...
        public override void Append(byte[] someBytes, bool asFrame = false)
        {
            ...
            {
                UInt64 dataLength;
                if (this.InternalState == DataState.Empty)
                {
