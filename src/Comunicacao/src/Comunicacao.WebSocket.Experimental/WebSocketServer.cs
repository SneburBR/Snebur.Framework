...
{
    public delegate void OnEventDelegate(SessaoContexto context);
    ...
    public class WebSocketServer : TcpServer, IDisposable
    {
        ...
        private static ConcurrentQueue<ConexaoContexto> ContextQueue { get; set; }
        private static Dictionary<ConexaoContexto, WebSocketServer> ContextMapping { get; set; }

        private static List<ConexaoContexto> CurrentConnections { get; set; }

        static WebSocketServer()
        {
            WebSocketServer.ContextQueue = new ConcurrentQueue<ConexaoContexto>();
            WebSocketServer.ContextMapping = new Dictionary<ConexaoContexto, WebSocketServer>();
            WebSocketServer.CurrentConnections = new List<ConexaoContexto>();

            WebSocketServer.CleanupThread = new Thread(HandleContextCleanupThread);
        ...
        private static void HandleContextCleanupThread()
        {
            while (!cancellation.IsCancellationRequested)
            {
                ...
                List<ConexaoContexto> currentConnections = new List<ConexaoContexto>();
        ...
        public override void Inicializar()
        {
            ...
            //cros domain
            //if (this.AccessPolicyServer == null)
            //{
            //    this.AccessPolicyServer = new AccessPolicyServer(this.ListenAddress, this.Origin, this.Porta);

            //    if (this.FlashAccessPolicyEnabled)
            //    {
            //        AccessPolicyServer.Inicializar();
            //    }
            //}
        }
        ...
        {
            //if ((this.AccessPolicyServer != null) && (this.FlashAccessPolicyEnabled))
            //{
            //    this.AccessPolicyServer.Parar();
            //    this.AccessPolicyServer = null;
            //}
            base.Parar();
        }
        ...
        protected override void ProcessarConexaoCliente(object data)
        {
            ...
            var context = new ConexaoContexto(this, connection);

            context.SessaoContexto.ClientAddress = context.Connection.Client.RemoteEndPoint;
            context.SessaoContexto.SetOnConnect(this.OnConnect);
            context.SessaoContexto.SetOnConnected(this.OnConnected);
            context.SessaoContexto.SetOnDisconnect(this.OnDisconnect);
            context.SessaoContexto.SetOnSend(this.OnSend);
            context.SessaoContexto.SetOnReceive(this.OnReceive);
            context.BufferSize = this.BufferSize;
            context.SessaoContexto.OnConnect();
        ...
        private void SetupContext(ConexaoContexto _context)
        {
            _context.ReceiveEventArgs.UserToken = _context;
            _context.ReceiveEventArgs.Completed += this.ReceiveEventArgs_Completed;
            _context.ReceiveEventArgs.SetBuffer(_context.Buffer, 0, _context.Buffer.Length);

            StartReceive(_context);
        }
        private void StartReceive(ConexaoContexto _context)
        {
            try
            {
                if (_context.ReceiveReady.Wait(this.TimeOut, cancellation.Token))
                {
                    try
                    {
