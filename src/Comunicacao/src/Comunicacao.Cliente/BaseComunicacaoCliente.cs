...
public abstract partial class BaseComunicacaoCliente : IBaseServico
{
    ...
    protected virtual CredencialUsuario CredencialUsuario
        => AplicacaoSnebur.AtualRequired.CredencialUsuario;
    ...
    protected T ChamarServico<T>(MethodBase? metodo, MethodBase? metodoParametros, object[] valoresParametro)
    {
        ...
        Guard.NotNull(metodoParametros);

        if (AplicacaoSnebur.AtualRequired.IsMainThread)
        {
        ...
        var chamdaServico = new ChamadaServico<T>(
            this.RetornarNomeManipulador(),
            contrato,
            this.UrlServico,
            tipoRetorno,
            this.ParametrosCabecalhoAdicionais);
    ...
    {
        var tipoRetorno = typeof(T);
        var chamadaAsync = new ChamadaServicoAsync(this.RetornarNomeManipulador(),
            this.RetornarContratoChamada(metodo, metodoParametros, true, valoresParametro),
            this.UrlServico,
            tipoRetorno,
            this.ParametrosCabecalhoAdicionais);
    ...
    {
        var metodo = this.GetType()
            .GetMethods()
            .Where(x => x.Name == nomeMetodo)
            .FirstOrDefault();
    ...
    public string RetornarContratoSerializado(MethodBase? metodo, params object[] parametros)
    {
        ...
        return JsonUtil.Serializar(contrato, EnumTipoSerializacao.DotNet, cultureInfo: null, isIdentar: true);
    }
    ...
    {
        return AplicacaoSnebur.AtualRequired.InformacaoSessao;
    }
    ...
    protected ContratoChamada RetornarContratoChamada(
        MethodBase? metodoChamada,
        MethodBase? metodoParametros,
        bool isAsync,
        params object?[] valoresParametro)
    {
        Guard.NotNull(metodoChamada);
        var informacaoSessaoUsuario = this.RetornarInformacaoSessoUsuarioRequisicaoAtual();
        informacaoSessaoUsuario.TipoAplicacao = AplicacaoSnebur.AtualRequired.TipoAplicacao;
        ...
        var parametrosChamada = this.RetornarParametrosChamada(
            metodoChamada,
            metodoParametros,
            isAsync,
            valoresParametro);
    ...
    protected virtual List<ParametroChamada> RetornarParametrosChamada(
        MethodBase? metodoChamada,
        MethodBase? metodoParametros,
        bool isAsync,
