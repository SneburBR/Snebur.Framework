...
public abstract partial class BaseComunicacaoServidor : IHttpHandler, IDisposable, IBaseServico
{
    ...
    private string RetornarResultadoChamadaSerializadoInterno(Requisicao requisicao, HttpContext httpContext)
    {
        ...
        try
        {
            Guard.NotNull(operacao);
            Guard.NotNull(parametros);
            ...
            return this.RetornarResultadoChamadaSerializadoInterno(
                parametros,
                operacao,
    ...
    private string RetornarResultadoChamadaSerializadoInterno(
        Dictionary<string, object?> dicionarParametros,
        string operacao,
    ...
    protected virtual object?[] RetornarParametrosMetodoOperacao(
        MethodInfo operacao,
        Dictionary<string, object?> parametros)
    {
        var parametrosOperacao = operacao.GetParameters();

        if (parametrosOperacao.Length != parametros.Count)
        {
        ...
        for (var i = 0; i <= parametrosOperacao.Length - 1; i++)
        {
            var parametroMetodo = parametrosOperacao[i];
            Guard.NotNullOrWhiteSpace(parametroMetodo.Name);

            var nomeParametroOperacao = NormalizacaoUtil.NormalizarNomeParametro(parametroMetodo.Name);
            ...
            if (valorParametro is null || (valorParametro.GetType().IsSubclassOf(typeof(BaseDominio)) ||
            ...
            else if (ReflexaoUtil.IsTipoRetornaColecao(parametroMetodo.ParameterType))
            {
                ...
                var lista = (Activator.CreateInstance(tipoListaTipado) as IList)
                    ?? throw new Exception("Não foi possível criar a lista do tipo " + tipoItemColecao.Name);
    ...
    private object RetornarObjetoBloqueioThreadCache(
        Dictionary<string, object?> parametros,
        string operacao,
        EnumTipoSerializacao tipoSerializacao)
    {
