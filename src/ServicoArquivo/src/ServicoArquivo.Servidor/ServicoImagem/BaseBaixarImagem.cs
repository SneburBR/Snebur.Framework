...
public abstract class BaseBaixarImagem : BaseBaixarArquivo<CabecalhoServicoImagem, IInformacaoRepositorioImagem>
{
    ...
    {
        if (informacaoRepositorioImagem is not CabecalhoServicoImagem cabecalhoServicoImagem)
        {
            throw new Exception($"O {nameof(informacaoRepositorioImagem)} deve ser do tipo {nameof(CabecalhoServicoImagem)}.");
        }
