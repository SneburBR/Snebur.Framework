...
public class ComunicacaoServicoArquivoCliente : BaseComunicacaoCliente, IComunicacaoServicoArquivo
{
    ...
    private string? IdentficadorProprietario { get; }
    ...
    protected override Dictionary<string, string> ParametrosCabecalhoAdicionais
    {
        get
        {
            ...
            var request = AplicacaoSnebur.AtualRequired.AspNetRequired.GetHttpContext<HttpContext>()?.Request;
            if (request != null)
            {
                ...
                {
                    parametros.Add(ConstantesCabecalho.IDENTIFICADOR_PROPRIETARIO, identificadorPropriedade.ToString());
                }
            }
            return parametros;
        }
    ...
    public ComunicacaoServicoArquivoCliente(
        string urlServico,
