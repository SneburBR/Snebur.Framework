...
public partial class ConsultaEntidade<TEntidade> where TEntidade : IEntidade
{
                                                     ...
                                                     bool isFiltro)
    {
        ...
        foreach (var p in propriedades)
        {
            ...
            Guard.NotNull(estruturaConsultaAtual.TipoEntidadeConsulta);
    ...
    private PropertyInfo NormalizarPropriedadeRelacaoEspecializda(PropertyInfo propriedade)
    {
        ...
        if (atributoPropriedadeEspecializada != null)
        {
            ...
            if (propridadeEspecializada is null)
            {
                throw new Exception($"A propriedade {propriedade.Name} com especializacao especializada  {atributoPropriedadeEspecializada.NomePropriedade} não foi encontrada,talvez não seja uma relação mapeaada.");
            }
    ...
    private void ValidarPropriedadeMapeamento(PropertyInfo propriedade,
                                              ...
                                              bool isFiltro)
    {
        var mapeamento = this.RetornarMapeamento(estruturaConsultaAtual,
                                                 caminhoPropreidade,
                                                 isFiltro);
        Guard.NotNull(mapeamento);
        Guard.NotNull(mapeamento.Propriedade);

        if (mapeamento.Propriedade.DeclaringType != propriedade.DeclaringType)
        {
            var tipos = new Type?[] { propriedade.DeclaringType,
                                     mapeamento.Propriedade.DeclaringType };
