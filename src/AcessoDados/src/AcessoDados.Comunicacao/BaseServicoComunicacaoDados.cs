...
public abstract class BaseServicoComunicacaoDados<TContextoDados> : BaseComunicacaoServidor, IDisposable where TContextoDados : BaseContextoDados
{
    ...
    {
        get => this.IsManterCache == false && field;
        protected set
        {
    ...
    protected override object? RetornarResultadoOperacao(
        ...
        object?[] parametros)
    {
        ...
        var status = this.ContextoDados?.SessaoUsuarioLogado?.Status ?? EnumStatusSessaoUsuario.Desconhecido;
    ...
    private string RetornarDetalhesParametros(
        MethodInfo metodoOperacao,
