...
public class TiposSeguranca
{
    ...
    {
        var propriedadesTipos = this.GetType().GetProperties()
            .Where(x => x.PropertyType == typeof(Type));

        var valoresPropriedade = propriedadesTipos
            .Select(x => x.GetValue(this) as Type)
            .ToList();
        return valoresPropriedade.All(x => x is not null);
    }
}