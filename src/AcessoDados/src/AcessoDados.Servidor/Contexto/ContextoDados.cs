...
public abstract partial class BaseContextoDados : __BaseContextoDados, IServicoDados, IContextoDadosSemNotificar, IDisposable
{
    ...
    internal Type? TipoEntidadeHistoricoMenutencao
        => this.EstruturaBancoDados.TipoHistoricoManutencao;
    ...
    public CredencialUsuario? CredencialAvalista
    {
        ...
        set
        {
            ...
            this.UsuarioAvalista = this.CacheSessaoUsuario?.RetornarUsuarioAvalista(field);
    ...
    {
        Guard.NotNull(configuracaoAcessoDados);
    ...
    protected virtual CacheSessaoUsuario RetornarCacheSessaoUsuario(
        BaseContextoDados baseContextoDados,
        CredencialUsuario credencial,
        Guid identificadorSessaoUsario,
        InformacaoSessao? informacaoSessaoUsuario)
    {
    ...
    internal EnumPermissao RetornarPermissaoLeitura(EstruturaConsulta estruturaConsulta)
    {
        ...
        Guard.NotNull(this.SeguracaContextoDados);
        ...
        bool ignorarValidacao)
    {
        ...
        using (var salvarEntidades = new SalvarEntidades(this,
            entidades.ToHashSet(false)!,
            EnumOpcaoSalvar.Salvar, false))
        {
    ...
    /// <summary>
    /// Retornar lista de usuários, que será automática salva no sistema, 
    /// </summary>
    /// <returns></returns>
    ...
    [MemberNotNull(nameof(TransacaoAtual))]
    [MemberNotNull(nameof(ConexaoAtual))]
    private void ValidarTransacaoAtual()
    {
        if (this.ConexaoAtual is null)
        {
            throw new Erro("A conexão atual da transação não foi inicializada");
        }
        if (this.TransacaoAtual is null)
        {
                ...
                params Expression<Func<TEntidade, object?>>[] expressoes)
    {
        ...
        if (!estruturaEntidaes.TryGetValue(tipoEntidade.Name,
