...
public partial class CacheSessaoUsuario
{
    private partial class AjudanteSessaoUsuarioInterno : IDisposable
    {
        ...
        private Type TipoUsuario { get; }

        private Type TipoSessaoUsuario { get; }

        private Type TipoIpInformacao { get; }
        ...
        {
            Guard.NotNull(estruturaBancoDados.TipoUsuario);
            Guard.NotNull(estruturaBancoDados.TipoSessaoUsuario);
            Guard.NotNull(estruturaBancoDados.TipoIpInformacao);
            ...
            this.UsuarioAnonimo = this.RetornarUsuario(contexto, CredencialAnonimo.Anonimo);
                                          ...
                                          Credencial credencial)
        {
            ...
            {
                return AplicacaoSnebur.AtualRequired.RetornarUsuario(contexto, credencialUsuario);
            }
            return null;
            ...
            else
            {
                ...
                if (this.IsSessaoUsuarioDiferente(contexto, sessaoUsuario, usuario))
                {
                    ...
                    var cloneSessao = (sessaoUsuario as Entidade)?.CloneSomenteId<Entidade>(true) as ISessaoUsuario
                         ?? throw new Exception("Não foi possível clonar a sessão do usuário");
        ...
        {
            var ip = AplicacaoSnebur.AtualRequired.AspNetRequired.IpRequisicao;
            ...
            var novoIpInformacao = Activator.CreateInstance(this.TipoIpInformacao) as IIPInformacao ??
                throw new Exception($"Não foi possível criar a instância do tipo {this.TipoIpInformacao}");
            try
            {
                ...
                Guard.InstanceOf<Entidade>(novoIpInformacao);
                contexto.Salvar((Entidade)novoIpInformacao);
        ...
        private IIPInformacao? RetornarIpInformacao(
        ...
        private ISessaoUsuario RetornarNovaSessaoUsuario(BaseContextoDados contexto,
                                                         ...
                                                         InformacaoSessao informacaoSessaoUsuario)
        {
            var sessaoUsuario = Activator.CreateInstance(this.TipoSessaoUsuario) as ISessaoUsuario
                ?? throw new Exception($"Não foi possível criar a instância do tipo {this.TipoSessaoUsuario}");
