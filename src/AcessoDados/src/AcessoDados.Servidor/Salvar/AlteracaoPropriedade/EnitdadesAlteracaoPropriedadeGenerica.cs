        ...
        foreach (var entidade in entidades)
        {
            ...
            foreach (var estruturaAlteracaoPropriedade in estruturaEntidade.TodasEstruturasAlteracaoPropriedadeGenerica)
            {
                ...
                if (isExisteAltracao)
                {
                    ...
                    Guard.NotNull(this.Contexto.EstruturaBancoDados.TipoEntidadeNotificaoPropriedadeAlteradaGenerica);

                    var novaAlteracao = Activator.CreateInstance(this.Contexto.EstruturaBancoDados.TipoEntidadeNotificaoPropriedadeAlteradaGenerica) as IAlteracaoPropriedadeGenerica
                                                   ?? throw new InvalidCastException($"O tipo {this.Contexto.EstruturaBancoDados.TipoEntidadeNotificaoPropriedadeAlteradaGenerica.Name} não pode ser convertido para {nameof(IAlteracaoPropriedadeGenerica)}");
