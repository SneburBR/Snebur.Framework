...
internal class AtualizarValorPadrao
{
        ...
        if (estruturasCamposValorPadrao?.Length > 0)
        {
            ...
            foreach (var estruturaCampo in estruturasCamposValorPadrao)
            {
                ...
                if (!entidade.__IsNewEntity &&
                    entidade.__PropriedadesAlteradas?.ContainsKey(propriedade.Name) == false)
                {
                    continue;
                }

                var valorPropriedade = propriedade.GetValue(entidade);
                var valorPadrao = RetornarValorPadraoPropriedade(
                    contexto,
                    estruturaCampo,
                    entidade,
                    propriedade,
    ...
    private static object? RetornarValorPadraoPropriedade(
        BaseContextoDados contexto,
        ...
        switch (tipoValorPadrao)
        {
            ...
            case EnumTipoValorPadrao.ValorPropriedadeNullOrDefault:
                ...
                if (propriedade.PropertyType.IsValueType)
                {
                    ...
                    if (valorDefault is not null && !valorDefault.Equals(valorPropriedadeTipado))
                    {
                        return null;
                    }
                    break;
            ...
            case EnumTipoValorPadrao.SessaoUsuario_Id:
                ...
                {
                    throw new Exception("O contexto não possui sessão de usuário logado para setar o valor padrão do id sessão do usuário");
                }
            ...
            case EnumTipoValorPadrao.UsuarioLogado_Id:
                ...
                if (!atributoUsuarioLogado.IsPermitirUsuarioAnonimo)
                {
                    ...
                    {
                        throw new Exception("O usuário anonimo não tem permissão para salvar setar o valor padrão do id usuario logado");
                    }
                }
                if (contexto.UsuarioLogado is null)
                {
                    throw new Exception("O contexto não possui usuário logado para setar o valor padrão do id usuario logado");
                }
