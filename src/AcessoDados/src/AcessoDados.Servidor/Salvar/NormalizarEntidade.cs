...
{
    internal HashSet<Entidade> Entidades { get; }
    internal BaseContextoDados Contexto { get; }
    internal Dictionary<string, Entidade> EntidadesNormalizadas { get; } = new();
    ...
    private void Normalizar(Entidade entidade)
    {
        if (!this.EntidadesNormalizadas.ContainsKey(entidade.__IdentificadorEntidade))
        {
            ...
            {
                var entidadeRelacaoPai = estruturaRelacaoPai.Propriedade.GetValue(entidade) as Entidade;
                if (entidadeRelacaoPai is not null)
                {
            ...
            {
                var entidadeRelacaoUmUm = estruturaRelacaoUmUm.Propriedade.GetValue(entidade) as Entidade;
                if (entidadeRelacaoUmUm is not null)
                {
            ...
            {
                var entidadeRelacaoUmUmReversa = estruturaRelacaoUmUmReversa.Propriedade.GetValue(entidade) as Entidade;
                if (entidadeRelacaoUmUmReversa is not null)
                {
            ...
            foreach (var estruturaRelacaoFilhos in estruturasRelacoesFilhos)
            {
                var entidadesFilho = estruturaRelacaoFilhos.Propriedade.GetValue(entidade) as IListaEntidades
                    ?? throw new Erro($"A propriedade {estruturaRelacaoFilhos.Propriedade.Name} da entidade {entidade.GetType().Name} não é uma lista de entidades");

                if (entidadesFilho.Count > 0)
                {
                    foreach (Entidade entidadeFilho in entidadesFilho)
                    {
                        Guard.NotNull(estruturaRelacaoFilhos.EstruturaRelacaoPai);

                        var relacaoPai = estruturaRelacaoFilhos.EstruturaRelacaoPai.Propriedade.GetValue(entidadeFilho) as IEntidade;
                        if (relacaoPai is not null)
                        {
