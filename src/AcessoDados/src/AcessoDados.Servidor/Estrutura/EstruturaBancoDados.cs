...
internal partial class EstruturaBancoDados
{
                                 ...
                                 BancoDadosSuporta sqlSuporte)
    {
        ...
        Guard.NotNull(this.AssemblyEntidades);

    }
    #endregion
    internal Type RetornarTipoConsultaImplementaInterface<TIInterface>()
    {
        return RetornarTipoConsultaImplementaInterface<TIInterface>(false)!;
    }
    internal Type? RetornarTipoConsultaImplementaInterface<TIInterface>(
        bool ignorarNaoEncontrado = false)
    {
        ...
        for (var i = 0; i < len; i++)
        {
            ...
            {
                while (tipoEntidade.BaseType is not null && ReflexaoUtil.IsTipoImplementaInterface(tipoEntidade.BaseType, tipoInterface, false))
                {
    ...
    private void MontarEstuturasEntidades(Type tipoContexto, BancoDadosSuporta sqlSuporte)
    {
        ...
        foreach (var tipoEntidade in tiposEntidadeConsulta)
        {
            ...
            if (tipoEntidade.BaseType?.IsSubclassOf(typeof(Entidade)) == true)
            {
    ...
    private void MontarEstruturaEntidade(Type tipoEntidade, BancoDadosSuporta sqlSuporte)
    {
        ...
        if (tipoEntidade.BaseType is not null &&
            !AjudanteEstruturaBancoDados.TipoEntidadeBaseNaoMepeada(tipoEntidade.BaseType))
        {
    ...
    private void AssociarEstruturasBase(EstruturaEntidade estruturaEntidade)
    {
        ...
        while (tipoEntidadeBase is not null &&
               !AjudanteEstruturaBancoDados.TipoEntidadeBaseNaoMepeada(tipoEntidadeBase))
        {
    ...
    private void PopularInterceptadores()
    {
        ...
        if (tipos.Count > 0)
        {
            ...
            {
                var tipoInterceptorGenerico = type.GetInterfaces()
                    .Where(x => x.IsGenericType && x.GetGenericTypeDefinition() == typeof(IInterceptador<,>))
                    .FirstOrDefault();

                if (tipoInterceptorGenerico is not null)
                {
                    return tipoInterceptorGenerico.GetGenericArguments().Last();
                }
