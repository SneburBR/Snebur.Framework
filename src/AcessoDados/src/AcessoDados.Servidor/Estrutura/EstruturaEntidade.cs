...
internal partial class EstruturaEntidade
{
    ...
    private readonly HashSet<string> _cacheValorNulos = new();
    ...
    internal EstruturaCampo? EstruturaCampoIdentificadorProprietario
             => this.RetornarEstruturaCampoRecursivo(ref field,
    ...
    internal EstruturaRelacaoFilhos[] TodasRelacoesFilhos
             => this.RetornarTodasEstruturasRelacaoRecursiva(ref field!);
    ...
    internal void AssociarEstruturaRalacaos()
    {
        ...
        {
            var estruturaRelacaoUmUm = estruturaRelacaoUmUmReversa
                .EstruturaEntidadeUmUmReversa.TodasRelacoesUmUm.
                                                                   Where(x => x.EstruturaEntidadeChaveEstrangeiraDeclarada == estruturaRelacaoUmUmReversa.EstruturaEntidade)
                                                                   .SingleOrDefault();
    ...
    private EstruturaCampo? RetornarEstruturaCampoRecursivo(
        ref EstruturaCampo? valorRef,
        Func<EstruturaEntidade, EstruturaCampo?> funcRetornarEstruturaCampo,
        [CallerMemberName] string callerMemberName = "")
    {
        if (valorRef is not null)
        {
            return valorRef;
        }

        if (this._cacheValorNulos.Contains(callerMemberName))
        {
            return null;
        }
        ...
        lock (this._lock)
        {
            var estruturaCampo = this.RetornarValorRecursivo(funcRetornarEstruturaCampo);
            valorRef = estruturaCampo;
            if (estruturaCampo is null)
            {
                this._cacheValorNulos.Add(callerMemberName);
                return null;
            }
    ...
    private T? RetornarValorRecursivo<T>(
        Func<EstruturaEntidade, T?> funcRetornarEstruturaCampo)
    {
    ...
    private T[] RetornarEstruturasCamposRecursivo<T>(
        ref T[] valoresRef,
        Func<EstruturaEntidade, T?[]?> funcRetornarEstruturaCampo,
        [CallerMemberName] string callerMemberName = "")
        where T : EstruturaPropriedade
    {
        if (!this.EstruturaBancoDados.IsEstruturasEntidadeMontada)
        {
            throw new Exception("A  estrutura do banco de dados não foi montada");
        ...
        lock (this._lock)
        {
            ...
            while (estruturaEntidadeatual != null)
            {
                ...
                Guard.NotNull(valoresAtual);

                valores.AddRangeNotNull(valoresAtual);
                estruturaEntidadeatual = estruturaEntidadeatual.EstruturaEntidadeBase;
            }
            valoresRef = valores.ToArray();
            return valoresRef;
        }
    }

    private TEstruturaRelacao[] RetornarTodasEstruturasRelacaoRecursiva<TEstruturaRelacao>(
        ref TEstruturaRelacao[] valoresRef,
        [CallerMemberName] string callerMameberName = "") where TEstruturaRelacao : EstruturaRelacao
    {
        if (!this.EstruturaBancoDados.IsEstruturasEntidadeMontada)
        {
            throw new Exception("A  estrutura do banco de dados não foi montada");
        ...
        lock (this._lock)
        {
            ...
            valoresRef = estruturasRelacao.ToArray();
            return valoresRef;
        }
    }
    ...
    {
        Guard.NotNull(this.EstruturasCamposValorPadraoInsertInterno);
    ...
    private void AdicionarEstruturaRelacaoFilhos(
        PropertyInfo propriedade,
        DicionarioEstrutura<EstruturaEntidade> estruturasEntidade)
    {
    ...
    {
        return this.EstruturasCampos.Values
            .Where(x => ReflexaoUtil.IsPropriedadePossuiAtributo(x.Propriedade, typeof(ValorPadraoIDUsuarioLogadoAttribute)))
            .SingleOrDefault();
    }
    ...
    private int RetornarMaximoRegistroPorConsulta()
    {
        ...
        return MAXIMO_REGISTRO_POR_CONSULTA;
    }
    ...
    private IInterceptador? RetornarInterceptador(
        EstruturaBancoDados estruturaBancoDados,
        EstruturaEntidade? estruturaEntidadeBase)
    {
