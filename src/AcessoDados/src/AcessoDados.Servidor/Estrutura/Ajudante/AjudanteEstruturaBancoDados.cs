...
internal class AjudanteEstruturaBancoDados
{
    ...
    {
        var atributoTable = tipo.GetCustomAttribute<TabelaAttribute>(false);
        if (atributoTable is null)
        {
    ...
    {
        var atributoTable = tipo.GetCustomAttribute<TableAttribute>(false);
        if (atributoTable is null)
        {
                                                            ...
                                                            bool ignorarPropridadesClasseBase = true)
    {
        ...
        if (propriedades.Count == 0)
        {
            ...
            {
                return RetornarPropriedadeRelacao(tipoEntidade, tipoPropriedadeRelacao, tipoRelacaoEnum, false);
            }
        }
    ...
    internal static string RetornarNomePropriedadeChaveEstrangeira(PropertyInfo propriedade)
    {
        ...
        if (atributoChaveEstrangeira is null)
        {
    ...
    {
        var tipoGenerico = RetornarTipoGerencio(propriedade.PropertyType);
        if (tipoGenerico is not null)
        {
            return tipoGenerico;
        }
        ...
        {
            var tipoGenericoArray = propriedade.PropertyType.GetElementType();
            if (tipoGenericoArray is null)
            {
                throw new Exception(String.Format("Não foi possível determinar o tipo do array na propriedade {0} ", propriedade.Name));
            }
        }

        throw new Exception(String.Format("O tipo da propriedade não é uma lista ou array {0} ", propriedade.PropertyType.Name));

    }

    private static Type? RetornarTipoGerencio(Type? tipoGenerico)
    {
        if (tipoGenerico is null)
        {
            return null;
        }
        ...
        {
            return RetornarTipoGerencio(tipoGenerico.BaseType);
        }
    }
    ...
    {
        var propriedadesCampo = RetornarPropriedadesCampos(tipoTipoComplexo);
