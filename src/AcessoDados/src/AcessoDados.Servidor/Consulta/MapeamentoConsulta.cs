...
internal partial class MapeamentoConsulta : BaseMapeamentoConsulta
{
    ...
    private List<IdTipoEntidade> RetornarIdsTipoEntidadeMuitosIds(
        MapeamentoEntidade mapeamentoEntidade,
        FiltroMapeamentoIds filtroMapeamento)
    {
        var ids = filtroMapeamento.Ids;
        var menorId = ids.Min();
        var maiorId = ids.Max();

        var filtroEntre = new FiltroMapeamentoEntre(
            filtroMapeamento,
