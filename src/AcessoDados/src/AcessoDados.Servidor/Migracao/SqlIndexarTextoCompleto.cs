...
internal class SqlIndexarTextoCompleto : SqlMigracao
{
    ...
    internal SqlIndexarTextoCompleto(EstruturaEntidade estruturaEntidade,
                                     List<PropertyInfo> propriedade) :
        base(estruturaEntidade, propriedade)
    {
        this.NomeCatalogo = this.RetornarNomeCatalogo();
        this.NomeChave = estruturaEntidade.TipoEntidade.GetCustomAttribute<NomeIndiceChavePrimariaAttribute>()?.Nome ??
                                 $"PK_{this.Schema}.{this.NomeTabela}";
    }
    ...
    private string RetornarSqlExisteIndice()
    {
        ...
        sb.AppendLine($" i.name = '{this.NomeChave}'  ");
        //sb.AppendLine($" and cl.name = '{this.NomeCampos}' ");
        sb.AppendLine(" )");
        return sb.ToString();
    }
